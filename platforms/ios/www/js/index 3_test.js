...
var appPage = function( page, video, poster, loop, sound, pausetime, doloop, frame, index, name ) {
	this.elem = page;
	this.video = video;
	this.poster = poster;
	this.videoloop = loop;
...
*/
						...
						_self.pages[_self.pageCount] = new appPage(newPage, video, poster, loop, sound, page.pause.time, page.pause.loop, newFrame, _self.pageCount, key );
				...
				_self.pageContainer.appendChild( _self.pages[0].elem );
				_self.parallaxContainer.appendChild( _self.pages[0].frame );
				_self.pageContainer.appendChild( _self.pages[1].elem );
				_self.parallaxContainer.appendChild( _self.pages[1].frame );
				_self.pageContainer.appendChild( _self.pages[2].elem );
				_self.parallaxContainer.appendChild( _self.pages[2].frame );
		...
		loadScroll: function( first ) {
			...
			if ( first ) _self.currentPageIndex = _self.currentArrayIndex = 1; // Go to 2nd page
		...
		renderPages: function(forward){
			//while (_self.pageContainer.firstChild) _self.pageContainer.removeChild(_self.pageContainer.firstChild);
			//while (_self.parallaxContainer.firstChild) _self.parallaxContainer.removeChild(_self.parallaxContainer.firstChild);
			
			var newIndex = (forward) ? _self.currentArrayIndex+1 : _self.currentArrayIndex-1;
			if ( forward ) {
				console.log( "forward" );
				newIndex = ( newIndex >= _self.pageCount ) ? 0 : newIndex;
				
				_self.pageContainer.removeChild( _self.pageContainer.firstChild );
				_self.parallaxContainer.removeChild( _self.parallaxContainer.firstChild );
				
				_self.pageContainer.appendChild( _self.pages[newIndex].elem );
				_self.parallaxContainer.appendChild( _self.pages[newIndex].frame );
			} else {
				newIndex = ( newIndex >= _self.pageCount ) ? 0 : newIndex;
				
				_self.pageContainer.removeChild( _self.pageContainer.lastChild );
				_self.parallaxContainer.removeChild( _self.parallaxContainer.lastChild );
				
				_self.pageContainer.insertBefore( _self.pages[1].elem, _self.pageContainer.childNodes[0] );
				_self.parallaxContainer.insertBefore( _self.pages[1].frame, _self.parallaxContainer.childNodes[0] );
			}
			_self.currentPageIndex = 1;
			_self.loadScroll( false );
			_self.myScroll.goToPage(1, 0, 0);
			_self.initCurrentPage(1);
		...
		onScrollEnd: function() {
			...
			if ( _self.currentPageIndex != lastPageIndex ) {
				_self.renderPages( _self.currentPageIndex > lastPageIndex );
			}
		},
